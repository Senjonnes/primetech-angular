...
export const DealType: any[] = ['Buy', 'Sell'];
...
export const Settlement: any[] = ['Deliverable', 'Cash'];
...
export class FinancialData {
  /*** Generates additional financial data fields */
  public static generateData(count: number): any[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      dataObj['OpenPriceDiff'] =
        ((dataObj['Open Price'] - dataObj['Price']) / dataObj['Price']) *
        100 *
        150;
      dataObj['BuyDiff'] =
        ((dataObj['Buy'] - dataObj['Price']) / dataObj['Price']) * 100 * 150;
      dataObj['SellDiff'] =
        ((dataObj['Sell'] - dataObj['Price']) / dataObj['Price']) * 100 * 150;
      dataObj['Start(Y)Diff'] =
        ((dataObj['Start(Y)'] - dataObj['Price']) / dataObj['Price']) *
        100 *
        150;
      dataObj['High(Y)Diff'] =
        ((dataObj['High(Y)'] - dataObj['Price']) / dataObj['Price']) *
        100 *
        150;
      dataObj['Low(Y)Diff'] =
        ((dataObj['Low(Y)'] - dataObj['Price']) / dataObj['Price']) * 100 * 150;
      dataObj['High(D)Diff'] =
        ((dataObj['High(D)'] - dataObj['Price']) / dataObj['Price']) *
        100 *
        150;
      dataObj['Low(D)Diff'] =
        ((dataObj['Low(D)'] - dataObj['Price']) / dataObj['Price']) * 100 * 150;
  ...
  public static generateHierarchicalData(count: number): any[] {
    ...
    for (let i = 0; i < Contract.length; i++) {
      ...
      for (const childDataObj of childData) {
        childDataObj[categories] = this.addHierarchicalData(
          childData / 4,
          i,
          childDataObj.ID,
          4
  ...
  private static addMockData(dataObj: any) {
    for (const mockData of MOCKFINANCEDATA) {
      for (const prop in mockData) {
        if (mockData.hasOwnProperty(prop)) {
          dataObj[prop] = mockData[prop];
  ...
  private static addHierarchicalData(
    count: number,
    i: number,
    parentRowId: number,
    toAdd?: number
  ...
  private static generateNewPrice(oldPrice: any): any {
  ...
  private static generateRandomNumber(min: any, max: any) {
